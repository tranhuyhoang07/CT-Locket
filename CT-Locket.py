...
class CTLocket:
    ...
    def excute(self, url, headers=None, payload=None, thread_id=None, step=None, proxies_dict=None):
        ...
        try:
            ...
            return None
    def setup(self):
        self._CTLocket_panel_()
    def _input_(self, prompt_text="", section="config"):
        print(
            f"{xColor.CYAN}┌──({xColor.NEON_GREEN}root@CT-Locket{xColor.CYAN})-[{xColor.PURPLE}{section}{xColor.CYAN}]")
    ...
    def _CTLocket_panel_(self):
        ...
        if CTLocket_confirm.strip().lower() in ('y', 'yes', '1'):
            self.CTLocket_confirm=True
        else:
...
def main():
    ...
    typing_print(
        f"-----------------[CTLocket Tool Pro {config.VERSION_TOOL}]-----------------", delay=0.01)
    config._print(
    ...
    if not config.FIREBASE_API_KEY:
        config._print(
            f"{xColor.RED}[!] {xColor.YELLOW}Critical error: Missing locket api key, please contact to discord @{config.discord}")
        config._loader_("Emergency shutdown initiated", 1.2)
        sys.exit(1)
    if not config.FIREBASE_APP_CHECK:
        config._print(
            f"{xColor.RED}[!] {xColor.YELLOW}Critical error: Missing locket token, please contact to discord @{config.discord}")
        config._loader_("Emergency shutdown initiated", 1.2)
        sys.exit(1)
    try:
